...
import xw.DBConnection.TempTable;
...
public class ConsoleTable {
    ...
    public static String[][] convertNodeTableToString(Pair<Map<Integer, Node>, String> resultPair) {
        assert resultPair.two.equals("node");
        Map<Integer, Node> nodeData = resultPair.one;
        String [][] resultString= new String[nodeData.size()][3];

        int i = 0;
        List<Integer> keyList = new ArrayList<Integer>(nodeData.keySet());
        Collections.sort(keyList);
        for (int key : keyList){
            resultString[i][0] = "" + nodeData.get(key).id;
            resultString[i][1] = nodeData.get(key).name;
            resultString[i][2] = nodeData.get(key).labelList.toString();
    ...
    public static String[][] convertEdgeTableToString(Pair<Map<Integer, Edge>, String> resultPair){
        assert resultPair.two.equals("edge");
        Map<Integer, Edge> edgeData = resultPair.one;
        String[][] resultString = new String[edgeData.size()][4];

        int i = 0;
        List<Integer> keyList = new ArrayList<Integer>(edgeData.keySet());
        Collections.sort(keyList);
        for (int key : keyList){
            resultString[i][0] = "" + edgeData.get(key).edgeID;
            resultString[i][1] = "" + edgeData.get(key).fromID;
            resultString[i][2] = "" + edgeData.get(key).toID;
            resultString[i][3] = edgeData.get(key).labelList.toString();
    ...
    public ConsoleTable(String[] header, String[][] rows) {
        this.header = header;
        this.rows = rows;
    }
